...
async function runIntegrityCheck(gameDir, fullCheck) {
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('[integrity] Manifesto vazio ou sem chave "files". URL:', manifestUrl);
      emit('integrity-error', 'Manifesto vazio — não foi possível verificar. Verifique a conexão.');
      return;
    }
